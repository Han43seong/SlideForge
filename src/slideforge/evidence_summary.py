...
def _summarize_pptx(root: Path, summary: EvidenceSummary) -> None:
    ...
    supplemental_render = _summarize_pptx_render_qa(root, summary)
    ...
    if gate.exists():
        ...
        if isinstance(raw_gate, dict):
            ...
            for blocker in _string_list(raw_gate.get("blockers")):
                if supplemental_render and (
                    "PPTX export/render validation tool" in blocker
                    or "Renderer evidence requires approved pptx-glimpse" in blocker
                ):
                    continue
    ...
    if export.exists():
        ...
        if isinstance(raw, dict):
            ...
            for blocker in blockers:
                if supplemental_render and "Renderer evidence requires approved pptx-glimpse" in blocker:
                    continue
            ...
            if supplemental_render:
                section.update(supplemental_render)
            elif renderer_status != "available":
...
def _summarize_pptx_render_qa(root: Path, summary: EvidenceSummary) -> dict[str, Any] | None:
    report = _first_existing(root, ["pptx-visual-render-qa.json", "pptx-render-qa.json"])
    if report is None:
        return None
    summary.artifacts["pptx-render-qa"] = _rel(report, root)
    raw = _read_json(report, summary, _rel(report, root))
    if not isinstance(raw, dict):
        summary.warnings.append("pptx_render_qa_unreadable: supplemental PPTX render QA report is not a JSON object")
        return None
    render_artifacts = raw.get("render_artifacts", {})
    visual_qa = raw.get("visual_qa", {})
    blockers = _string_list(raw.get("blockers"))
    for blocker in blockers:
        summary.blockers.append(f"pptx_render_qa_blocker: {blocker}")
    for warning in _string_list(raw.get("warnings")):
        summary.warnings.append(f"pptx_render_qa_warning: {warning}")
    png_count = render_artifacts.get("png_count") if isinstance(render_artifacts, dict) else None
    svg_count = render_artifacts.get("svg_count") if isinstance(render_artifacts, dict) else None
    if isinstance(png_count, int):
        summary.counts["pptx_render_pngs"] = png_count
    if isinstance(svg_count, int):
        summary.counts["pptx_render_svgs"] = svg_count
    verdict = visual_qa.get("verdict") if isinstance(visual_qa, dict) else None
    korean_readable = visual_qa.get("korean_glyphs_readable") if isinstance(visual_qa, dict) else None
    tofu = visual_qa.get("missing_glyph_or_tofu_boxes") if isinstance(visual_qa, dict) else None
    blocked_layout = visual_qa.get("blocking_clipping_or_overlap") if isinstance(visual_qa, dict) else None
    if tofu:
        summary.blockers.append("pptx_render_qa_blocker: Korean glyphs rendered as tofu/missing boxes")
    if blocked_layout:
        summary.blockers.append("pptx_render_qa_blocker: blocking clipping or overlap detected in rendered PPTX")
    return {
        "renderer_status": "rendered",
        "renderer_qa_path": _rel(report, root),
        "visual_qa_verdict": verdict or "recorded",
        "korean_glyphs_readable": bool(korean_readable),
        "rendered_pngs": png_count or 0,
        "rendered_svgs": svg_count or 0,
    }
